-- =========================================================================
-- Extend create_completed_invoice to support sale_return / purchase_return
-- =========================================================================
...
BEGIN
  ...
  IF v_type = 'sale_return' AND v_method = 'credit' AND p_customer_id IS NULL THEN
    RAISE EXCEPTION 'العميل مطلوب لمرتجعات المبيعات الآجلة';
  END IF;

  IF v_type = 'purchase_return' AND v_method = 'credit' AND p_supplier_id IS NULL THEN
  ...
  -- For returns: cash means full paid; credit means remaining = total (paid=0)
  -- Keep same paid/remaining model as sale/purchase.
  ...
  INSERT INTO public.invoices (
    ...
    v_type,
    'completed',
    CASE WHEN v_type IN ('sale', 'sale_return') THEN p_customer_id ELSE NULL END,
    CASE WHEN v_type IN ('purchase', 'purchase_return') THEN p_supplier_id ELSE NULL END,
  ...
  LOOP
    ...
    v_unit_cost := COALESCE(
      ...
      CASE WHEN v_type IN ('purchase', 'purchase_return') THEN v_unit_price ELSE 0 END
    );
  ...
  -- Party balance: sale/purchase increase remaining; returns decrease
  IF v_remaining > 0 THEN
    IF v_type IN ('sale', 'sale_return') THEN
      PERFORM public.adjust_customer_balance(p_customer_id, v_balance_sign * v_remaining);
    ELSE
      PERFORM public.adjust_supplier_balance(p_supplier_id, v_balance_sign * v_remaining);
    END IF;
  END IF;
