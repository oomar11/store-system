-- =========================================================================
-- Offline sync: client_operations + occurred_at on invoice/safe/document seq
-- =========================================================================
...
CREATE INDEX IF NOT EXISTS idx_client_operations_user_occurred
  ON public.client_operations(user_id, occurred_at DESC);

ALTER TABLE public.client_operations ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS client_operations_select_own ON public.client_operations;
CREATE POLICY client_operations_select_own
  ON public.client_operations FOR SELECT TO authenticated
  USING (user_id = auth.uid() OR public.has_app_permission('audit'));

-- Claim or return existing result (idempotency for multi-step ops)
