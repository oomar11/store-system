-- Allow transfers involving soft-deleted or inactive safes (common for
-- pairs like الخزنة الرئيسية ↔ المحل after sync/UI toggles).
-- Desktop already listed them; mobile client filters were rejecting them.
-- SECURITY DEFINER can revive rows before moving cash.
...
BEGIN
  ...
  SET
    ...
    is_active = true,
    updated_at = NOW()
  WHERE id IN (p_from_safe_id, p_to_safe_id)
    AND (deleted_at IS NOT NULL OR COALESCE(is_active, true) = false);
  -- updated_at exists since local-first sync migration.
